    ...
    for num in keys:
        ...
        if len(R[num])>0:
            ...
            for i in range(len(R[num])):
                nb_pal += str(R[num][i])+'\n' 
                #on réouvre la connexion
                cnxn = pyodbc.connect("DRIVER={SQLite3 ODBC Driver};SERVER=localhost;DATABASE=plaplan.db;Trusted_connection=yes;")
                cursor = cnxn.cursor()
                colis = cursor.execute(''' select colis from planning_colis where pal/10 = ?''', R[num][i]).fetchval()
                print(colis)
                nb_colis += str(colis) + '\n'
                tot_colis += colis
        ...
        for j in range(len(pal[num])):
            if pal[num][j] not in R[num]:
                ...
                cursor.execute(''' delete from planning_colis
                                where pal/10 = ?''', pal[num][j])
        #on actualise le fichier de données
        cursor.commit()
        cnxn.close()  
