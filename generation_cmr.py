...
def controle_pal(shop_pal,pal,R):
    return(pal,shop_pal,R)
...
def gene_cmr(R,pal,shop_pal):
    ...
    for num in keys:
        ...
        # idx= shop_pal.index(num)
        reste_quai[num]=[]
        # for j in range(1,len(pal[idx][1:])+1):
        ...
        if len(R[num])>0:
            ...
            # p = document.add_paragraph()
            # p.add_run('8.Formalités de douane et autres/Douanes en andere formaliteiten').bold = True
            ...
            # win32api.ShellExecute (0, None,'C:/Users/Utilisateur/Documents/FNAC_VDB/Projet_expe/CMR_expé_shop_'+str(num)+date_CMR+'.docx', None, ".", 0)
            
            ##win32api.ShellExecute (0, "print", "CMR_expé_shop_"+str(num)+".docx", None, ".", 0)
            #win32api.ShellExecute (0, None, 'Liste_erreur_scan_shop_'+str(num)+'.docx', None, ".", 0)
