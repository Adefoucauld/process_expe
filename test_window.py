...
''' On fait test pour scan et actualisation en obdc des données'''
...
from generation_cmr_odbc import *
...
cnxn = pyodbc.connect("DRIVER={SQLite3 ODBC Driver};SERVER=localhost;DATABASE=plaplan.db;Trusted_connection=yes;")
cursor = cnxn.cursor()
...
df = pd.read_sql('''select id,shop, CAST(pal AS FLOAT) AS pal, colis from planning_colis''', cnxn)
df = df.fillna(0) 

#on crée une liste des shop
shop = cursor.execute('''select distinct shop from planning_colis''')
...
for row in shop:
    if row[0] != None:
        shop_pal.append(row[0])

    

for elem in shop_pal:
    palet= cursor.execute('''select cast(pal/10 as float) from planning_colis where shop = ? ;''',elem)
    l = [int(i[0]) for i in palet if i[0] != None]
    pal[int(elem)//10] = l
    
    
shop_pal = [x//10 for x in shop_pal]
